...
class CatAndMouse {
 public:
  ...
  /**
   * Update the mouse's and cat's position after mouse moves towards \p target.
   */
   ...
   *
   * After being called once, this will have no effect before a call to step(const std::complex<double>&) const.
   */
  void undo();
