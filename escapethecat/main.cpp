...
inline double dot(const std::complex<double>& a,
...
inline std::ostream& operator<<(std::ostream& os,
    ...
    return os << static_cast<int>(pos.real()) << ' '
              << static_cast<int>(pos.imag());
}
...
int main() {
    ...
    while (true) {
        ...
        std::transform(
                center_ticks.begin(), center_ticks.end(), mouse_ticks.begin(),
                [&m = cm.mouse()](const auto& tick) { return m + tick; });

        // Only consider targets towards boundary
        ticks.clear();
        std::copy_if(mouse_ticks.begin(), mouse_ticks.end(),
                     std::back_inserter(ticks), [&](const auto& tick) {
                         return dot(cm.cat() - cm.mouse(), tick - cm.mouse())
                                < 0;
                     });
