...
inline std::complex<double> next_mouse(const CatAndMouse& cm, const std::complex<double>& target) {
  /** Localize target. */
...
std::complex<double> next_cat(const CatAndMouse& cm) {
  ...
  const int sign = static_cast<int>(std::round(normalized_cat_angle / std::abs(normalized_cat_angle)));
